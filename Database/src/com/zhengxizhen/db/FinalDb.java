...
import com.zhengxizhen.db.sqlite.CursorUtils;
import com.zhengxizhen.db.sqlite.DbModel;
import com.zhengxizhen.db.sqlite.SqlBuilder;
import com.zhengxizhen.db.sqlite.SqlInfo;
import com.zhengxizhen.db.table.ManyToOne;
import com.zhengxizhen.db.table.OneToMany;
import com.zhengxizhen.db.table.TableInfo;
...
public class FinalDb {
	...
	private FinalDb(DaoConfig config) {
		...
		this.db = new SqliteDbHelper(config.getContext(), config.getDbName(),
	...
	public <T> void saveList(List<T> entitys) {
		...
		try {
			final int len = entitys.size();
			for (int i = 0; i < len; i++) {
				exeSqlInfo(SqlBuilder.buildInsertSql(entitys.get(i)));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
